...
private func requireAccessibility() throws {
    ...
    guard AXIsProcessTrusted() else {
        throw fail(
            ...
            "enable Accessibility for Conteenua Companion (Device Control on newer macOS), then fully quit and reopen the app"
        )
    }
}
...
private func requireScreenCapture() throws {
    guard CGPreflightScreenCaptureAccess() else {
        ...
        throw fail(
            ...
            "enable Screen Recording for Conteenua Companion, then fully quit and reopen the app"
        )
    }
}
...
private func captureImage(filter: SCContentFilter, configuration: SCStreamConfiguration) throws -> CGImage {
    ...
    do {
        try stream.addStreamOutput(output, type: .screen, sampleHandlerQueue: DispatchQueue(label: "conteenua-companion.capture"))
    } catch {
