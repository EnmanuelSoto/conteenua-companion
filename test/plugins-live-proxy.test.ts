...
/** Opt-in network acceptance: actual npm server, actual SDK client and actual Conteenua Companion HTTP proxy. */
it.runIf(process.env.COS_PLUGIN_LIVE_TEST === '1')('routes upstream Memory through Plugins HTTP, records exact ownership and refuses a cached disabled call', async () => {
  ...
  try {
    ...
    client = new Client({ name: 'Conteenua Companion live acceptance', version: '1.0.0' });
    ...
    const mutation = await client.callTool({ name, arguments: { entities: [{ name: 'Conteenua Companion proxy acceptance', entityType: 'test', observations: ['Local disposable fixture'] }] } });
    expect(mutation.isError).not.toBe(true);
    const read = installed.tools.find(tool => tool.name === 'read_graph')!.exposedName;
    expect(JSON.stringify(await client.callTool({ name: read, arguments: {} }))).toContain('Conteenua Companion proxy acceptance');
    ...
    expect(JSON.stringify(calls)).toContain('Conteenua Companion proxy acceptance');
  } finally {
    await client?.close();
    await endpoint?.stop();
