...
describe('who is allowed to talk to it', () => {
  ...
  it('identifies itself to an extension without any credential', async () => {
    ...
    expect(reply.status).toBe(200);
    expect(reply.body.app).toBe('conteenua-companion');
...
describe('delivering a bootstrap', () => {
        ...
        events: [
          {
            kind: 'user_message',
            time: Date.now(),
            text: 'Continue the previous Conteenua Companion session. Read the handoff below.',
  ...
  it.each(['worker', 'resume'] as const)('adds setup only to a new worker, never a resumed chat (%s)', async kind => {
    ...
    if (kind === 'worker') {
      ...
      expect(command.text).not.toContain('[[CONTEENUA_CONTEXT:');
  ...
  it('keeps a redeemed revival browser-owned until its exact sent acknowledgement', async () => {
    ...
    expect(claimed.body.command.text).not.toContain('[[CONTEENUA_CONTEXT:');
    ...
    await events(PRIME, [
      {
        kind: 'user_message',
        time: Date.now(),
        text: '[[CLF-RESUME:O8THi8gMTC6LvH9GLclDIQ]]\n\nContinuing a Conteenua Companion session that was compacted.',
