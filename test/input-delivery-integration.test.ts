...
it('delivers only the selected project AGENTS.md, freezes claims across restart, and budgets the Astra appendix before cutting', async () => {
  ...
  expect(tool.messages[0]!.text).not.toMatch(/PROJECT_CHANGED|CONTEENUA_CONTEXT/);
...
describe('IPC input delivery and Goal control integration', () => {
  ...
  it('requires an exact plugin claim and matching schema before a refresh completion', async () => {
    ...
    publishPluginSurface('core', 'Conteenua Companion Core', 'test', 'Synthetic instructions', tools);
    ...
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'Conteenua Companion Core', tools })).body.ok).toBe(false);
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'Conteenua Companion Core', tools: [{ ...tools[0], description: 'Old declaration' }] })).body.ok).toBe(true);
  ...
  it('defaults automatic plugin refresh off and revokes an already offered claim without removing the backend', async () => {
    ...
    plugin.publishPluginSurface('core', 'Conteenua Companion Core', 'test', '', tools);
    ...
    const claim = { action: 'claim', id: saved.id, appId: 'asdk_app_off_on_test', connectorName: 'Conteenua Companion Core', tools: [{ ...tools[0], description: 'Older declaration' }] };
  ...
  it('accepts a manual plugin-refresh terminal state and removes it from browser pickup', async () => {
    ...
    const manual = await post('/plugin-refresh', { ...request, appId: 'asdk_app_synthetic', action: 'manual', connectorName: 'Conteenua Companion Core', tools: installed, error: 'Recreate or republish this custom app.' });
