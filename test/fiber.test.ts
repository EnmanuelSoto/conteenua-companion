 ...
 */
const APP = 'Conteenua Companion Core';
const DESKTOP_APP = 'Conteenua Companion Desktop';
...
describe('reading a row out of the page', () => {
  ...
  it.each(['Conteenua Companion Core', 'Conteenua Companion Desktop', 'Conteenua Companion Plugins', 'Conteenua Companion Backup', 'Gmail'])(
    'keeps result-only metadata scoped to the exact supported connector %s', async app => {
      ...
      expect(single.turns[0]!.calls).toHaveLength(['Conteenua Companion Backup', 'Gmail'].includes(app) ? 0 : 1);
    });
...
describe('the calls a turn says it made', () => {
  ...
  /**
   * A name is not a prefix game. `Conteenua Companion Backup` shares every character of the
  ...
  it('refuses a connector whose name merely starts with this app’s brand', async () => {
    const messages = [
      request('req-fake', 'read', { app: 'Conteenua Companion Backup' }),
      answer('res-fake', 'req-fake', 'read', 'Conteenua Companion Backup'),
