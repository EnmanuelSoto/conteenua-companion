...
function toolCall(seq: number, callId: string): SessionEvent {
  return {
    ...
    call: {
      ...
      result: text('# Conteenua Companion'),
      outcome: 'ok',
      durationMs: 40,
...
it('reorders whole project groups without changing chat selection, ownership or disclosure across refresh', async () => {
  ...
  const saved = w.localStorage.getItem('conteenua-companion.sidebar-order');
  ...
  expect(w.localStorage.getItem('conteenua-companion.sidebar-order')).toBe(saved);
