...
describe('tunnel binary location', () => {
  it('finds the repository tunnel bundle from both source and compiled development layouts', () => {
    const repo = path.join(os.tmpdir(), 'conteenua-companion-layout');
    const expected = path.normalize(path.join(repo, 'resources', 'tunnel'));
    const sourceDir = path.join(repo, 'src', 'main', 'tunnel');
    const compiledDir = path.join(repo, 'out', 'main');

    expect(bundledTunnelDirs('', sourceDir)).toContain(expected);
    expect(bundledTunnelDirs('', compiledDir)).toContain(expected);
  });
