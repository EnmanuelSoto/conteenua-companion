...
it('preserves the entire Unicode prompt and literal boundary-like user text across both readers', () => {
  ...
  try {
    ...
    for (const text of [authored, sent.slice(0, 80), sent.replace('CONTEENUA_CONTEXT:', 'CONTEENUA_CONTEXT:9')]) {
...
it('hides a provider-prefixed blank paragraph using exact source, retaining strict frames and authored whitespace', () => {
  ...
  try {
    ...
    for (const invalid of [source.replace('CONTEENUA_CONTEXT:', 'CONTEENUA_CONTEXT:9'), source.slice(0, 70), '  Ordinary request\n[[/CONTEENUA_CONTEXT]]']) {
      source = invalid;
