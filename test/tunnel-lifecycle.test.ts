...
describe('cloudflared public readiness', () => {
  const cloudflaredSettings = {
  ...
  it('does not report connected until the public MCP URL answers initialize', async () => {
    vi.useFakeTimers();
    const reports: any[] = [];
    ...
    const handle = await startTunnel({
      localUrl: 'http://127.0.0.1:1234/mcp/core/secret',
    ...
    try {
      const child = fixture.children[0];
      child.stderr.emit('data', Buffer.from('Your quick Tunnel has been created! Visit it at https://working-one.trycloudflare.com\n'));
      await vi.advanceTimersByTimeAsync(500);
      expect(reports.some(report => report.state === 'connected')).toBe(false);
      expect(reports.at(-1)).toMatchObject({
        state: 'connecting-tunnel',
        publicUrl: 'https://working-one.trycloudflare.com/mcp/core/secret'
      });
  ...
  it('retires an unreachable quick-tunnel hostname instead of showing a false connected state', async () => {
    vi.useFakeTimers();
    const reports: any[] = [];
    vi.stubGlobal('fetch', vi.fn(async () => {
    ...
    try {
      const first = fixture.children[0];
      first.stderr.emit('data', Buffer.from('https://dead-one.trycloudflare.com\n'));
      await vi.advanceTimersByTimeAsync(12_000);

      expect(reports.some(report => report.state === 'connected')).toBe(false);
      expect(reports.some(report => String(report.detail).includes('Retrying'))).toBe(true);
      expect(fixture.terminate).toHaveBeenCalledWith(first.pid);
