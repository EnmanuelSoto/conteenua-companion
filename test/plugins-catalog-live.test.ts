...
for (const id of ['blender', 'unity', 'playwright']) {
  it.runIf(process.env.COS_PLUGIN_LIVE_TEST === '1')(`installs ${id} and verifies its advertised tool preview over stdio`, async () => {
    ...
    const client = new Client({ name: 'Conteenua Companion catalog discovery acceptance', version: '1.0.0' });
    let transport: StdioClientTransport | undefined;
    const page = createServer((_request, response) => { response.setHeader('Content-Type', 'text/html'); response.end('<h1>Conteenua Companion plugin browser fixture</h1>'); });
    try {
      ...
      if (id === 'playwright') {
        ...
        expect(JSON.stringify(content)).toContain('Conteenua Companion plugin browser fixture');
      }
    } finally {
