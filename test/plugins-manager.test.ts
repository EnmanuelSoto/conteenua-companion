...
readline.createInterface({input:process.stdin}).on('line',line=>{const m=JSON.parse(line);if(m.id===undefined)return;let result;if(m.method==='initialize')result={protocolVersion:'2025-11-25',capabilities:{tools:{}},serverInfo:{name:'Conteenua Companion test fixture',version:'1'}};else if(m.method==='tools/list')result={tools};else if(m.method==='tools/call')result={content:[{type:'text',text:process.env.TEST_SECRET||m.params.arguments.value}],structuredContent:{value:m.params.arguments.value},isError:m.params.arguments.value==='error'};else result={};process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:m.id,result})+'\\n');});`;
...
readline.createInterface({input:process.stdin}).on('line',line=>{
  ...
  it('rejects plaintext configuration declared sensitive by a replacement bundle', async () => {
    ...
    const manifest = {
      manifest_version: '0.3', name: 'sensitive-fixture', version: '1.0.0', description: 'Test', author: { name: 'Conteenua Companion' },
  ...
  it('launches an imported MCPB relative Node entry point with stable data cwd', async () => {
    ...
    const manifest = {
      ...
      author: { name: 'Conteenua Companion' },
      server: {
        type: 'node',
  ...
  it.runIf(process.env.COS_PLUGIN_LIVE_TEST === '1')(
    ...
    async () => {
      ...
      const written = await manager.call(create.exposedName, {
        entities: [
          { name: 'Conteenua Companion plugin validation fixture', entityType: 'test', observations: ['End-to-end plugin routing'] },
        ],
      });
      expect(written.isError).not.toBe(true);
      expect(JSON.stringify(await manager.call(read.exposedName, {}))).toContain('Conteenua Companion plugin validation fixture');
      await manager.update(row.id);
