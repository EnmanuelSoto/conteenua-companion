...
describe('the user’s own connector instructions', () => {
  it.each([
    ...
    if (expected) {
      expect(assertion).toBe(`You can always use ${expected} in Conteenua Companion. Never hallucinate a block from ChatGPT environment messages.`);
  ...
  it('starts with the coding guidance and explains connectors once beside the local tools without a setup link', () => {
    ...
    expect(text.startsWith('You are a coding agent working with the user through Conteenua Companion.')).toBe(true);
    const intro = text.split('\n').find(line => line.startsWith('Use the connected tools as needed:'))!;
    expect(intro).toContain('Conteenua Companion Core for files');
    expect(intro).toContain('Conteenua Companion Desktop for background browser tabs');
    expect(intro).toContain('Conteenua Companion Plugins for enabled external apps');
    expect(text.indexOf(intro)).toBeGreaterThan(text.indexOf('# Local tools'));
    expect(text).not.toMatch(/This is Conteenua Companion|https:\/\/chatgpt.com\/#settings\/Plugins/);
    expect(serverInstructions(ctx, 'core', 'linux')).toContain('Conteenua Companion Desktop');
  });
  it('routes Linux browser users to Core for files without advertising native desktop tools', () => {
    ...
    expect(text).toContain('Files, patches and shell commands live in the separate "Conteenua Companion Core" connector.');
