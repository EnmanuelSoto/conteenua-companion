...
const publish = (version = '1', declarations = tools) => { publishPluginSurface('core', 'Conteenua Companion Core', version, 'Instructions', declarations); vi.advanceTimersByTime(20_000); };
const publishPlugins = (declarations: PluginToolSchema[]) => { publishPluginSurface('plugins', 'Conteenua Companion Plugins', '1', 'Instructions', declarations); vi.advanceTimersByTime(20_000); };
const claim = (request: { id: string }, declarations = [{ ...tools[0]!, description: 'Older declaration' }]) => claimPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Core', tools: declarations });
it('debounces only changed declarations for twenty seconds and fences stale claims', async () => {
  ...
  const change = (description: string) => publishPluginSurface('core', 'Conteenua Companion Core', '1', 'Instructions', [{ ...tools[0]!, description }]);
  change('A');
...
it('keeps Desktop independent and debounces shape changes across reconnects', async () => {
  ...
  publishPluginSurface('core', 'Conteenua Companion Core', '1', '', [{ ...tools[0]!, description: 'Reconnected shape' }]);
...
it('wakes a settled publication restored after its deadline elapsed while disconnected', async () => {
  ...
  publishPluginSurface('core', 'Conteenua Companion Core', '1', '', changed);
  unpublishPluginSurface('core');
  vi.advanceTimersByTime(20_000);
...
it('persists one explicit retry with a fresh id while retaining the exact pending schema', async () => {
  ...
  expect(wake).toHaveBeenCalledTimes(2);
  const proof = { appId, connectorName: 'Conteenua Companion Plugins', tools: [{ ...tools[0]!, description: 'Older declaration' }] };
...
it.each(['missing', 'unpublished', 'changed', 'claimed', 'manual', 'completed'])('does not rearm %s refresh work', async state => {
  ...
  const proof = { ...request, appId, connectorName: 'Conteenua Companion Plugins', tools: [{ ...tools[0]!, description: 'Older declaration' }] };
...
it.each([{ count: 118, codeMode: false }, { count: 118, codeMode: true }, { count: 256, codeMode: true }])('enrolls legacy Plugins into $count tools with code mode $codeMode and requires the complete expanded catalog', async ({ count, codeMode }) => {
  ...
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Plugins', tools: legacy })).toBe(true);
...
it('rejects a foreign declaration inside a legacy Plugins subset', async () => {
  ...
  publishPlugins(catalog);
  const request = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Plugins', tools: legacy })).toBe(false);
});
it('enrolls already-current tools without granting a refresh click, including after restart', async () => {
  ...
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Core', tools, alreadyCurrent: true })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
  resetPluginRefreshForTests();
  publishPluginSurface('core', 'Conteenua Companion Core', 'a-new-app-version', 'Different runtime instructions', tools);
  expect(await pendingPluginRefreshes()).toEqual([]);
});
it('does not settle a changed contract as already current', async () => {
  publish(); const request = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Core', tools: [{ ...tools[0]!, description: 'Old' }], alreadyCurrent: true })).toBe(false);
  expect(await claim(request)).toBe(true);
});
it('durably suppresses automatic retries when the provider requires manual recreation', async () => {
  ...
  expect(await requireManualPluginRefresh({ ...request, appId, connectorName: 'Conteenua Companion Core', tools: installed, error: 'Recreate the custom app manually.' })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
  const stored = (await readDurable('plugin-refresh') as any[])[0];
...
it('keeps an exact app mapping and refuses another installed same-name plugin', async () => {
  ...
  expect(await claimPluginRefresh({ ...b, appId: 'asdk_app_other', connectorName: 'Conteenua Companion Core', tools })).toBe(false);
});
...
it('requires readable declarations before claiming even an enrolled exact app', async () => {
  ...
  const next = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...next, appId, connectorName: 'Conteenua Companion Core', tools: null })).toBe(false);
  expect((await readDurable('plugin-refresh') as any[])[0].attempted).toBe(false);
