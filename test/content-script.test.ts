...
describe('the app-owned chronological stream', () => {
  ...
  it.each(['unmounted', 'clipped', 'expanded'])('keeps an interrupted response readable without a Thinking disclosure (%s)', async mode => {
    ...
    section.setAttribute('data-clf-fiber-turn', '0');
    await replyFiber([{
      v: 21, index: 0, messageId: 'interim-native-X', tool: 'read', app: 'Conteenua Companion Core', answered: true,
      conversationId: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'
  ...
  it('keeps prior local calls mounted while a new exact app call is still native', async () => {
    ...
    const rows = (secondAnswered: boolean) => [
      { v: 21, index: 0, messageId: 'fiber-one', tool: 'read_file', path: '/Conteenua Companion Core/read_file',
        app: 'Conteenua Companion Core', answered: true, conversationId: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee' },
      { v: 21, index: 1, messageId: 'fiber-two', tool: 'exec_command', path: '/Conteenua Companion Core/exec_command',
        app: 'Conteenua Companion Core', answered: secondAnswered, conversationId: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee' }
    ];
  ...
  it.each([
    ['Conteenua Companion Plugins', true],
    ['Conteenua Companion Backup', false]
  ...
  it.each([false, true])('hides result-only native duplicates only after mounted coverage for a shared request (same tool: %s)', async sameTool => {
    ...
    await replyFiber(calls.map((call, index) => ({ v: 21, index, ...call, path: null,
      app: 'Conteenua Companion Core', resource: `/asdk_app_fixture/link_fixture/${call.tool}`,
  ...
  it('restores a native connector row when a preserved moved root loses current suppression proof', async () => {
    ...
    const bind = async (answered: boolean) => replyFiber([{
      ...
      path: '/Conteenua Companion Core/read_file', app: 'Conteenua Companion Core', answered,
      conversationId: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'
...
describe('the Compact & resume control', () => {
  ...
  it('says what the job is doing at every stage', async () => {
    ...
    expect(state({ disconnected: true })).toMatchObject({
      mode: 'off',
      hint: 'Browser connection is disconnected in Conteenua Companion.',
      action: 'none'
    });
  ...
  it('preserves a stale COS handoff draft including user edits and retires the unsent ticket', async () => {
    ...
    const stale =
      ...
      'Conteenua Companion is compacting this conversation so a fresh chat can continue the work. Stop whatever you were doing and do only this.\n\n' +
...
describe('folding away the chat’s opening instruction', () => {
  ...
  it('previews the authored prompt from its exact frame rather than its folded DOM aggregate', async () => {
    ...
    const framed = `\n\n[[CONTEENUA_CONTEXT:${instructions.length}]]\n${instructions}\n[[/CONTEENUA_CONTEXT]]\n\nRead the fixture.`;
...
describe('the fresh chat the app opened', () => {
  ...
  it('sends a worker bootstrap whose task is shorter than the text it verifies', async () => {
    ...
    // run died this way.
    const task = 'Read /project/conteenua-companion/package.json and report the version field.';
    live = await harness(
...
(You are a worker agent in a Conteenua Companion multi-agent run.)`,
            agent: 'worker-1'
          }
        }),
        ack: () => ({ ok: true })
      ...
      (document, dom) => {
        document.querySelector('[data-testid="send-button"]')!.addEventListener('click', () => {
          ...
          userTurn(document, 'accepted-short-worker', `${task}\n\n(You are a worker agent in a Conteenua Companion multi-agent run.)`, { sent: false });
        });
      }
    );
