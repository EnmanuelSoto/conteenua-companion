...
const safeEmail = '227782719+legacy-maintainer@users.noreply.github.com';
...
function commit(repository: string, message: string, email: string): void {
  execFileSync('git', ['commit', '--allow-empty', '-m', message], {
    ...
    env: {
      ...
      GIT_COMMITTER_NAME: 'legacy-maintainer',
...
function verify(repository: string, args: string[] = []) {
  return spawnSync(process.execPath, [script, ...args], {
    ...
    encoding: 'utf8',
    windowsHide: true,
    env: { ...process.env, GIT_AUTHOR_NAME: 'legacy-maintainer', GIT_AUTHOR_EMAIL: safeEmail },
  });
}
...
describe('public-history privacy gate', () => {
  it.each(['\\', '\\\\', '/'])('rejects private roots using %s in staged and committed content without echoing them', separator => {
    ...
    const privateRoot = ['C:', 'Users', 'legacyuser'].join(separator);
  ...
  it('rejects a non-noreply maintainer identity without printing the address', () => {
    const repository = makeRepository();
    const privateEmail = ['legacymaintainer', 'gmail.com'].join('@');
  ...
  it.each([
    'https://github.com/legacy-maintainer/conteenua-companion.git',
    'git@github.com:legacy-maintainer/conteenua-companion.git',
    'ssh://git@github.com/legacy-maintainer/conteenua-companion'
    ...
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    commit(repository, 'Already public canonical commit', ['legacymaintainer', 'gmail.com'].join('@'));
    ...
    commit(repository, 'New unpublished unsafe identity', ['legacymaintainer', 'gmail.com'].join('@'));
    expect(verify(repository).status).toBe(1);
  });

  it.each([
    'https://github.com/example/conteenua-companion.git',
    'https://github.com/legacy-maintainer/conteenua-companion-extra.git',
    'https://github.com.example/legacy-maintainer/conteenua-companion.git'
  ])('does not trust an unrelated upstream URL (%s)', (url) => {
    const repository = makeRepository();
    execFileSync('git', ['remote', 'add', 'upstream', url], { cwd: repository });
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    commit(repository, 'Unpublished identity', ['legacymaintainer', 'gmail.com'].join('@'));
  ...
  it('does not fall back to fork history when canonical main has not been fetched', () => {
    const repository = makeRepository();
    execFileSync('git', ['remote', 'add', 'upstream', 'https://github.com/legacy-maintainer/conteenua-companion.git'], { cwd: repository });
    commit(repository, 'Only published on a fork', ['legacymaintainer', 'gmail.com'].join('@'));
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    expect(verify(repository).status).toBe(1);
  });
