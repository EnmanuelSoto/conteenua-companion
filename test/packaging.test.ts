...
const { CLOUDFLARED, RIPGREP, TUNNEL_CLIENT } = packagingVersions;
const {
...
describe('cross-platform packaging targets', () => {
  ...
  it('pins tunnel-client, cloudflared and ripgrep for every release OS/CPU pair', () => {
    for (const platform of SUPPORTED_PLATFORMS) {
      for (const arch of SUPPORTED_ARCHES) {
        expect(TUNNEL_CLIENT.targets[platform][arch].sha256).toMatch(/^[0-9a-f]{64}$/);
        expect(CLOUDFLARED.targets[platform][arch].sha256).toMatch(/^[0-9a-f]{64}$/);
        expect(CLOUDFLARED.targets[platform][arch].asset).toMatch(/^cloudflared-/);
    ...
    expect(CLOUDFLARED.version).toBe('2026.9.1');
    expect(CLOUDFLARED.targets.win32.arm64.asset).toBe('cloudflared-windows-amd64.exe');
  ...
  it('assembles every platform artifact in the reusable release workflow', () => {
    ...
    expect(matrix).toEqual([
      {
        ...
        script: 'dist:x64', artifact: 'package-windows-x64', files: 'release/Conteenua-Companion-Setup-x64.exe'
      },
      {
        name: 'Windows arm64', platform: 'win32', arch: 'arm64', runner: 'windows-11-arm',
        script: 'dist:arm64', artifact: 'package-windows-arm64', files: 'release/Conteenua-Companion-Setup-arm64.exe'
      },
      {
        name: 'macOS x64', platform: 'darwin', arch: 'x64', runner: 'macos-15-intel',
        script: 'dist:mac:x64', artifact: 'package-macos-x64',
        files: 'release/Conteenua-Companion-macOS-x64.dmg\nrelease/Conteenua-Companion-macOS-x64.zip\n'
      },
      {
        name: 'macOS arm64', platform: 'darwin', arch: 'arm64', runner: 'macos-15',
        script: 'dist:mac:arm64', artifact: 'package-macos-arm64',
        files: 'release/Conteenua-Companion-macOS-arm64.dmg\nrelease/Conteenua-Companion-macOS-arm64.zip\n'
      },
      {
        name: 'Linux x64', platform: 'linux', arch: 'x64', runner: 'ubuntu-24.04',
        script: 'dist:linux:x64', artifact: 'package-linux-x64',
        files: 'release/Conteenua-Companion-Linux-x64.AppImage\nrelease/Conteenua-Companion-Linux-x64.deb\n'
      },
      {
        name: 'Linux arm64', platform: 'linux', arch: 'arm64', runner: 'ubuntu-24.04-arm',
        script: 'dist:linux:arm64', artifact: 'package-linux-arm64',
        files: 'release/Conteenua-Companion-Linux-arm64.AppImage\nrelease/Conteenua-Companion-Linux-arm64.deb\n'
      }
    ]);
    expect(parsed.jobs.package['runs-on']).toBe('${{ matrix.runner }}');
    expect(workflow).toContain('name: conteenua-companion-candidate-${{ github.run_id }}');
    ...
    expect(workflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a /usr/bin/conteenua-companion');
    ...
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$mount_dir/Conteenua Companion.app\"");
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$zip_dir/Conteenua Companion.app\"");
    ...
    expect(workflow).toContain("grep -Fxq 'Name=Conteenua Companion' \"$desktop\"");
    expect(workflow).toContain("grep -Fxq 'Icon=conteenua-companion' \"$desktop\"");
    ...
    expect(debGui).toContain('xvfb-run -a /usr/bin/conteenua-companion');
  ...
  it('uses Noble-compatible Linux packages and a FUSE-independent AppImage runtime', () => {
    ...
    expect(builder.linux.artifactName).toBe('Conteenua-Companion-Linux-${env.COS_PACKAGE_ARCH}.${ext}');
    ...
    expect(builder.linux.maintainer).toBe('Conteenua <support@conteenua.com>');
    expect(pkg.desktopName).toBe('com.conteenua.companion.desktop');
    expect(pkg.homepage).toBe('https://github.com/EnmanuelSoto/conteenua-companion');
    ...
    expect(releaseWorkflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a /usr/bin/conteenua-companion');
    ...
    expect(releaseWorkflow).toContain("test \"$(dpkg-deb --field \"$deb\" Package)\" = conteenua-companion");
    ...
    expect(releaseWorkflow).toContain('test -L /usr/bin/conteenua-companion');
    expect(releaseWorkflow).toContain('installed_executable="$(readlink -f /usr/bin/conteenua-companion)"');
  ...
  it('pins the current macOS release to unsigned thin native bundles with explicit metadata checks', () => {
    ...
    expect(builder.mac.artifactName).toBe('Conteenua-Companion-macOS-${arch}.${ext}');
    ...
    for (const marker of [
      "CFBundleIdentifier: 'com.conteenua.companion'",
      "CFBundleExecutable: 'Conteenua Companion'",
      "CFBundleName: 'Conteenua Companion'",
      "CFBundleDisplayName: 'Conteenua Companion'",
  ...
  it('hides Electron helper parentheses from otool-classic without changing the inspected file', () => {
    const file = '/Applications/Conteenua Companion.app/Contents/Frameworks/Conteenua Companion Helper (GPU).app/Contents/MacOS/Conteenua Companion Helper (GPU)';
...
`;
  ...
  it('keeps version metadata aligned and validates artifacts independently of editorial release notes', () => {
    ...
    const artifacts = [
      ...
      'Conteenua-Companion-macOS-x64.zip',
      'Conteenua-Companion-macOS-arm64.dmg',
      'Conteenua-Companion-macOS-arm64.zip',
      'Conteenua-Companion-Linux-x64.AppImage',
      'Conteenua-Companion-Linux-x64.deb',
      'Conteenua-Companion-Linux-arm64.AppImage',
      'Conteenua-Companion-Linux-arm64.deb',
