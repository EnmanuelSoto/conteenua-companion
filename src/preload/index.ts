...
const api = {
  ...
  conteenuaCloudStatus: () => call<ConteenuaCloudStatus>('conteenuaCloud:status'),
  conteenuaCloudSignIn: (email: string, password: string) => call<ConteenuaCloudStatus>('conteenuaCloud:signIn', { email, password }),
  conteenuaCloudSignOut: () => call<ConteenuaCloudStatus>('conteenuaCloud:signOut'),
  conteenuaCloudSync: () => call<ConteenuaCloudStatus>('conteenuaCloud:sync'),
  onConteenuaCloudChanged: (listener: (status: ConteenuaCloudStatus) => void): (() => void) => {
    const wrapped = (_event: unknown, status: ConteenuaCloudStatus): void => listener(status);
    ipcRenderer.on('conteenuaCloud:changed', wrapped);
    return () => ipcRenderer.removeListener('conteenuaCloud:changed', wrapped);
  },
