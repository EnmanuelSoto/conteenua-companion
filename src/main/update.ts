/**
 * Whether a newer Conteenua Companion has been published, and the file that becomes it.
 *
...
const REPO = 'EnmanuelSoto/conteenua-companion';
...
export function stagedArtifact(
  ...
  if (platform === 'win32') return { name: `Conteenua-Companion-Setup-${arch}.exe`, kind: 'installer', target: '' };
  if (platform === 'linux' && appImage) {
    return { name: `Conteenua-Companion-Linux-${arch}.AppImage`, kind: 'appimage', target: appImage };
  }
  return null;
}
...
async function get(url: string, timeout: number, headers: Record<string, string> = {}): Promise<Response> {
  const response = await fetch(url, {
    ...
    redirect: 'follow',
    headers: { 'user-agent': `conteenua-companion/${APP_VERSION}`, ...headers }
  });
