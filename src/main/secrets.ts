...
const LINUX_STORAGE_PROBE = 'conteenua-companion-safe-storage-probe';
 ...
 */
export type SecretKey = 'openaiApiKey' | 'bridgeToken' | 'openRouterApiKey' | 'customProviderApiKey' | `plugin:${string}` | `setup:${string}` | `conteenua:${string}`;
    ...
    if (platform === 'linux') {
      ...
      if (!secureStorageCiphertextIsProtected(probe, platform)) {
        return {
          available: false,
          detail:
            'Linux secure storage fell back to Electron’s insecure hard-coded-key provider. Start or unlock a desktop keyring/Secret Service (for example GNOME Keyring or KWallet), then restart Conteenua Companion.'
        };
      }
    }
