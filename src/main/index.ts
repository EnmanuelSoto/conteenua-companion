...
import { initializeConteenuaCloud, registerConteenuaCloudIpc, shutdownConteenuaCloud } from './conteenua-cloud.js';

// Private-beta/E2E isolation only. Packaged releases always use Electron's normal userData
// directory; an environment variable can never redirect a production install's credentials or
// permission state. Development verification uses this to avoid touching the user's real profile.
const developmentUserData = process.env.CONTEENUA_COMPANION_USER_DATA?.trim();
if (!app.isPackaged && developmentUserData) app.setPath('userData', path.resolve(developmentUserData));
...
function createWindow(): void {
  ...
  if (process.env.ELECTRON_RENDERER_URL) {
    ...
    void window.loadFile(path.join(__dirname, '../renderer/companion.html'));
  }
}
...
function refreshTray(): void {
  ...
  tray.setToolTip(`Conteenua Companion — ${label.toLowerCase()}`);
  tray.setContextMenu(
...
void app.whenReady().then(async () => {
  ...
  registerConteenuaCloudIpc((channel, payload) => {
    const target = window;
  ...
  logInfo('app started');
  void initializeConteenuaCloud().catch(error => logWarn(`Conteenua cloud control: ${error instanceof Error ? error.message : String(error)}`));
...
app.on('will-quit', (event) => {
  ...
  void runShutdownSequence(
    [
      ...
      { name: 'admission/drain', budgetMs: 40_000, run: () => [shutdownConnection(), shutdownBridge(), shutdownConteenuaCloud()] },
