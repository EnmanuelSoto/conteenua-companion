 ...
 */
export function bundledTunnelDirs(
  resourcesPath = process.resourcesPath ?? '',
  moduleDir = __dirname
): string[] {
  // `electron-vite build` bundles every main-process module into `out/main/index.js`, so the
  // runtime __dirname is `out/main`, not `src/main/tunnel`. Keep both layouts explicit:
  //   source/tests: src/main/tunnel -> ../../../resources/tunnel
  //   compiled dev: out/main       -> ../../resources/tunnel
  // Packaged builds still win through Electron's resourcesPath.
  const candidates = [
    resourcesPath ? path.join(resourcesPath, 'tunnel') : '',
    path.resolve(moduleDir, '..', '..', 'resources', 'tunnel'),
    path.resolve(moduleDir, '..', '..', '..', 'resources', 'tunnel')
  ].filter(Boolean);
...
function bundledDir(): string | null {
  return bundledTunnelDirs().find(candidate => existsSync(candidate)) ?? null;
}
