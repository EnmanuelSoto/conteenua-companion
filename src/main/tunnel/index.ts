...
async function probePublicMcp(url: string): Promise<ProbeResult> {
  const controller = new AbortController();
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: 'conteenua-companion-readiness',
        method: 'initialize',
        params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'conteenua-companion', version: '0.1.0' } }
      }),
      signal: controller.signal
    });
    const body = await res.text().catch(() => '');
    const looksLikeMcp = /jsonrpc|serverInfo|protocolVersion|conteenua-companion-readiness/i.test(body);
    return { ok: res.ok && looksLikeMcp, detail: body.trim().slice(0, 200) || `HTTP ${res.status}` };
  } catch (error) {
    const cause = error && typeof error === 'object' && 'cause' in error ? (error as { cause?: unknown }).cause : null;
    const code = cause && typeof cause === 'object' && 'code' in cause ? String((cause as { code?: unknown }).code) : '';
    return { ok: false, detail: `${error instanceof Error ? error.message : String(error)}${code ? ` (${code})` : ''}` };
  } finally {
    clearTimeout(timer);
  }
}
...
async function startCloudflared(opts: TunnelStartOptions): Promise<TunnelHandle> {
  ...
  let attempt = 0;
  let currentRun: { child: ChildProcess; retired: boolean; timer: NodeJS.Timeout; lastError: string } | null = null;

  const retire = async (run: NonNullable<typeof currentRun>, detail: string, retry: boolean): Promise<void> => {
    if (run.retired || currentRun !== run) return;
    run.retired = true;
    clearTimeout(run.timer);
    await stopTree(run.child);
    if (stopped || currentRun !== run) return;
    currentRun = null;
    if (retry && attempt < MAX_ATTEMPTS) {
      opts.report({ state: 'connecting-tunnel', detail: `Cloudflare tunnel was not reachable. Retrying (${attempt + 1}/${MAX_ATTEMPTS})…`, publicUrl: null });
      await delay(1_000);
      if (!stopped) launch();
      return;
    }
    opts.report({ state: 'tunnel-unavailable', detail, publicUrl: null });
  };

  const verifyPublic = async (run: NonNullable<typeof currentRun>, publicUrl: string): Promise<void> => {
    let lastProbe = '';
    for (let probeAttempt = 0; probeAttempt < 10; probeAttempt += 1) {
      if (stopped || run.retired || currentRun !== run) return;
      const result = await probePublicMcp(publicUrl);
      if (result.ok) {
        run.retired = true;
        clearTimeout(run.timer);
        logInfo('quick tunnel connected');
        opts.report({ state: 'connected', detail: 'Connected. Paste the URL below into ChatGPT as a custom connector.', publicUrl });
        return;
      }
      lastProbe = result.detail;
      if (probeAttempt < 9) await delay(1_000);
    }
    const detail = lastProbe ? `Cloudflare returned a tunnel URL, but it never became reachable: ${lastProbe}`
      : 'Cloudflare returned a tunnel URL, but it never became reachable.';
    logWarn(`cloudflared readiness: ${detail}`);
    await retire(run, detail, true);
  };

  const launch = (): void => {
    attempt += 1;
    opts.report({ state: 'connecting-tunnel', detail: attempt === 1 ? 'Starting cloudflared…' : `Starting cloudflared retry ${attempt}/${MAX_ATTEMPTS}…`, publicUrl: null });
    ...
    const run = {
      ...
      retired: false,
      lastError: '',
      timer: setTimeout(() => undefined, 45_000)
    };
    clearTimeout(run.timer);
    run.timer = setTimeout(() => {
      if (!stopped && currentRun === run && !run.retired) {
        void retire(run, run.lastError || 'cloudflared did not produce a reachable public MCP URL within 45 seconds.', true);
      }
    }, 45_000);
    run.timer.unref?.();
    currentRun = run;
    ...
    const handleLine = (line: string): void => {
      ...
      if (match && !candidateSeen) {
        candidateSeen = true;
        const publicUrl = `${match[0]}${local.pathname}`;
        opts.report({ state: 'connecting-tunnel', detail: 'Cloudflare assigned a URL. Verifying public MCP reachability…', publicUrl });
        void verifyPublic(run, publicUrl);
        return;
      }
      if (/\berr\b|\berror\b|\bfatal\b/i.test(line)) {
        run.lastError = line.slice(0, 400);
        logWarn(`cloudflared: ${run.lastError}`);
      }
    };
    ...
    child.on('exit', (code) => {
      if (stopped || run.retired || currentRun !== run) return;
      void retire(run, run.lastError || `cloudflared stopped (exit ${code}).`, true);
    });
    child.on('error', (err) => {
      if (stopped || run.retired || currentRun !== run) return;
      void retire(run, `Could not start cloudflared: ${err.message}`, true);
    });
  };
