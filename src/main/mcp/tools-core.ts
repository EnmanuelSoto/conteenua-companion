...
import { gateRemoteCommandApproval } from '../conteenua-cloud.js';
      ...
      async ({ paths, start_line, end_line, max_bytes }) =>
        guard('read', async () => {
          if (!caps.read && !caps.browse && !caps.metadata) {
            return fail(
              'TOOL_DISABLED: read is disabled by the current Conteenua Companion permissions. Ask the user to enable reading in the app.'
            );
          }
    ...
    reg.register(
      ...
      async ({ path }) =>
        guard('view_image', async () => {
          if (!caps.read) {
            return fail(
              'TOOL_DISABLED: view_image is disabled by the current Conteenua Companion permissions. Ask the user to enable reading in the app.'
            );
          }
      ...
      async ({ patch }) =>
        guard('apply_patch', async () => {
          if (!caps.create && !caps.edit && !caps.move && !caps.deleteFile) {
            return fail(
              'TOOL_DISABLED: apply_patch is disabled by the current Conteenua Companion permissions. Ask the user to enable changing files in the app.'
            );
          }
    ...
    reg.register(
      ...
      async (input) =>
        reg.guarded('command', 'exec_command', async () => {
          ...
          const remoteApproval = await gateRemoteCommandApproval(currentCaller().sessionId, rawCommands);
          if (!remoteApproval.allowed) return fail(remoteApproval.message);
