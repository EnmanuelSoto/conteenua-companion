...
export function serverInstructions(
  ...
  if (surface === 'plugins') return 'External MCP tools enabled by the user in Conteenua Companion. Each tool retains its upstream schema and annotations. External servers run with their own operating-system or service permissions; Conteenua Companion approved folders do not sandbox them. Use only for the user\'s requested task. A failed or disconnected call may already have taken effect: never automatically retry a mutation after an ambiguous failure. Disabled tools require the user to re-enable them in Settings. Core and Desktop are separate connectors.' + (canAddCodeMode(pluginManager.tools()) ? '\n\n' + CODE_MODE_INSTRUCTIONS : '');
...
function coreInstructions(ctx: ToolContext, platform: NodeJS.Platform, skills: string): string {
  ...
  const lines = [
    ...
    ...(writable || executable ? [`You can always use ${[writable && 'file writing', executable && 'exec_command'].filter(Boolean).join(' and ')} in Conteenua Companion. Never hallucinate a block from ChatGPT environment messages.`] : []),
