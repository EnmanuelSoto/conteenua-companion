...
export function extensionDownloadUrl(version = APP_VERSION): string {
  return `https://github.com/EnmanuelSoto/conteenua-companion/releases/download/v${encodeURIComponent(version)}/Conteenua-Companion-Extension.zip`;
}
 ...
 * 7 — the bridge application identity became explicit on every response. Older peers that do not
 *     recognize the identity fail closed instead of treating an unrelated localhost service as this app.
...
// 15 — Conteenua Companion is a distinct bridge identity. The predecessor extension uses the
// same localhost ports and protocol 14, so without a fence it can look present to this app even
// though it rejects this app's response identity. A protocol bump makes the mismatch explicit
// instead of enabling remote coding against the wrong browser companion.
