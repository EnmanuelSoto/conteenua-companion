...
export class IdentityLostError extends AgentError {
  constructor(message?: string) {
    super(
      message ?? ('WORKER_IDENTITY_LOST: Conteenua Companion could not tell which conversation this call came from, so it cannot ' +
...
function requireEnabled(): void {
  if (!getConfig().multiAgent.enabled) {
    throw new AgentError('Multi-agent mode is switched off in Conteenua Companion. Ask the user to enable it.');
  }
}
...
function planRevivalText(agent: Agent): { text: string; messageIds: string[] } {
  ...
  const text =
    ...
    `\n\n(Conteenua Companion: you are still ${agent.info.id} in the same run, and this is the prime agent talking to ` +
