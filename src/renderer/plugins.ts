...
async function mutate(work: ReturnType<typeof window.api.pluginsSnapshot>, notify = true): Promise<boolean> {
  ...
  if (notify) toast(t("Plugin settings saved. Refresh the Conteenua Companion Plugins connector in ChatGPT to update its tools."));
  return true;
}
...
export function applyPluginsState(next: AppState): void {
  ...
  ui($('pluginsSetupHint'), 'textContent', () => configured
    ...
    : t("Add the Conteenua Companion Plugins connector in ChatGPT once so it can use your installed plugins."));
...
function showConnection(): void {
  ...
  copy(t("Connector name"), surface?.connectorName ?? 'Conteenua Companion Plugins');
  copy(t("Description"), surface?.description ?? 'Tools from your enabled Conteenua Companion plugins.');
...
function showUninstall(plugin: PluginView): void {
  ...
  body.append(el('p', '', () => t("This stops its connection and deletes the installation, its Conteenua Companion-managed local data and stored credentials. Back up any plugin data you need first. External application data is not removed.")), button(() => t("Uninstall plugin"), async () => { if (await mutate(window.api.pluginsUninstall(plugin.id))) box.close(); }, true));
}
...
function showCatalog(): void {
  ...
  body.append(custom, el('p', 'hint', () => t("Local plugins run as your OS user and do not inherit Conteenua Companion approved-folder restrictions. Install only code you trust.")));
}
...
function showCustom(kind: PluginSource['kind'], path = ''): void {
  ...
  body.append(el('p', 'hint', () => t("Remote servers must support MCP Streamable HTTP. GitHub links require a known recipe or supported manifest. Local servers run outside the Conteenua Companion folder sandbox.")), button(() => t("Install and connect"), async () => {
