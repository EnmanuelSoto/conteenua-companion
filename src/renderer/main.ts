...
function paintDesktopAccess(next: AppState): void {
  ...
  ui($('desktopAccessDetail'), 'textContent', () => t("{0}. These are live verdicts from the native backend executing inside Conteenua Companion. ", [missing.join(' · ')]) +
  ...
  if (update.latest) {
    ...
    lines.push(
      update.stage === 'checking'
        ...
        : update.stage === 'ready'
        ? t("Conteenua Companion {0} is downloaded and ready. Install it now, or it installs the next time you quit.", [update.latest])
        : update.stage === 'downloading'
          ? t("Conteenua Companion {0} is downloading. Keep working; you can install it when it lands.", [update.latest])
          : update.stage === 'failed'
            ? t("Conteenua Companion {0} could not be downloaded: {1}.", [update.latest, update.error ?? t("the download stopped")])
            : t("Conteenua Companion {0} is out. This installation has to be updated by hand.", [update.latest])
    );
    ...
    lines.push(t("Up to date! Conteenua Companion {0}{1}", [update.current, extension]));
    tone = 'ok';
  }
  if (stale) {
    lines.push(
...
function installUpdate(): void {
  toast(t("Installing the update. Conteenua Companion closes and starts again as the new version."));
