...
var CLF_DOM = (() => {
  ...
  function userPromptText(value) {
    ...
    const header = /^\[\[CONTEENUA_CONTEXT:(\d{1,6})\]\]\n/.exec(value.slice(identity.length));
   ...
   *
   * Conteenua Companion remains authoritative for its own call labels/results. This only supplies
   ...
   * recording "Conteenua Companion Desktop is now connected" as a ChatGPT failure.
   */
  ...
  async function pluginRefreshView(connectorName, expectedTools = [], expectedAppId = null) {
    const externalPlugins = connectorName === 'Conteenua Companion Plugins';
