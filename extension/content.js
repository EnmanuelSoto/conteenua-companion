...
(() => {
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[Conteenua Companion: browser observation truncated to fit transport.]';
  ...
  function reportMessages(nowGenerating) {
    ...
    for (const message of rendered) {
      ...
      if (message.role === 'user') {
        ...
        if (!source.canonical && /^\[\[CONTEENUA_CONTEXT:\d{1,6}\]\]/.test(source.text) && CLF_DOM.userPromptText(source.text) === null) continue;
   ...
   *
   * Exact names, never a prefix: `Conteenua Companion Backup` would be somebody else's
   * connector, and a prefix test would have this app vouch for its traffic.
   */
  const OUR_CONNECTORS = [
    'Conteenua Companion Core',
    'Conteenua Companion Desktop',
    'Conteenua Companion Plugins',
    'TobisComputer'
  ];
  ...
  function controlState(input) {
    ...
    if (!connected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'Conteenua Companion is not running on this PC.',
        action: 'none'
      };
    }
    if (!conversationId) {
  ...
  function buildControl() {
    ...
    blocked.setAttribute(
      'data-clf-tip',
      'This chat is blocked in the Conteenua Companion app: its tool calls are refused and Goal, Loop and auto-compaction are off. To release it, open the app’s Chat tab, hover this chat in the sessions list and press its block symbol.'
    );
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', 'Conteenua Companion settings');
    root.hidden = true;
  ...
  function renderControl() {
    ...
    control.button.setAttribute('aria-label', 'Conteenua Companion settings');
  ...
  function replyError(reply) {
    ...
    if (reply.error === 'app_not_found') return 'Conteenua Companion is not running on this PC.';
  ...
  */
  ...
  const noteStopClick = (event) => {
    ...
    // programmatic Conteenua Companion/recovery click a human action. Keep observing the turn.
    stopRequestedAt = Date.now();
  };
