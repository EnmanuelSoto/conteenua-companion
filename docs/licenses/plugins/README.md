...
These exact upstream texts support the reviewed catalog. They are not the complete dependency notices for an installed plugin: Conteenua Companion downloads those packages only when the user installs them, and retains their dependency directories and bundled code notices. They do not change the MIT license of Conteenua Companion. The bundled SVG illustrations are MIT artwork, not upstream logos.
