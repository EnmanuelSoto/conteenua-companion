...
let appExecutable;
if (targetPlatform === 'darwin') {
  const appBundle = path.join(packageRoot, 'Conteenua Companion.app');
  resourcesDir = path.join(appBundle, 'Contents', 'Resources');
  appExecutable = path.join(appBundle, 'Contents', 'MacOS', 'Conteenua Companion');
} else {
  resourcesDir = path.join(packageRoot, 'resources');
  appExecutable = path.join(packageRoot, targetPlatform === 'win32' ? 'Conteenua Companion.exe' : 'conteenua-companion');
}
