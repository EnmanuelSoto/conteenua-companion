...
const notices = [
  'Conteenua Companion — Third-party Notices',
  '',
  'Conteenua Companion is MIT licensed; see LICENSE. This inventory preserves the license and notice texts',
  ...
  '',
  'Catalog artwork: Copyright (c) 2026 Conteenua Companion contributors. MIT licensed, see LICENSE.',
  ...
  'are not bundled with Conteenua Companion; their package directories retain their own licenses and notices.',
  ''
];
const missing = [];
notices.push('='.repeat(80), 'OpenAI Codex — adapted coding instructions and update_plan contract',
  ...
  'Conteenua Companion adapts identity and available tools, removes Codex-specific facilities and adds bounded plan details and session storage.', '');
for (const file of ['LICENSE', 'NOTICE']) {
  notices.push(`--- Codex ${file} ---`, await fs.readFile(path.join(root, 'docs/licenses/codex', file), 'utf8'), '');
}
notices.push('='.repeat(80), 'Cloudflare cloudflared 2026.9.1',
  'Source: https://github.com/cloudflare/cloudflared/tree/2026.9.1',
  'Declared license: Apache-2.0', '');
notices.push('--- cloudflared LICENSE ---', await fs.readFile(path.join(root, 'docs/licenses/cloudflared-LICENSE'), 'utf8'), '');
let count = 0;
...
notices.push('='.repeat(80), 'Optional plugin catalog — separate installations and hosted services', 'Reviewed package licenses and hosted-service references follow. External server code is not bundled with Conteenua Companion; custom installations and updates retain their own notices.', '');
