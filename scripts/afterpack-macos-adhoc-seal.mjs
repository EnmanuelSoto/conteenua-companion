 ...
 *
 *     codesign --verify --deep --strict "Conteenua Companion.app"
...
export default async function sealMacOsBundle(context) {
  ...
  // never uses those APIs. Conteenua Companion denies renderer permission requests and has no media-capture
  // feature, so shipping those declarations is misleading and can make macOS surface unrelated
  // privacy prompts. Keep only privacy declarations for capabilities Conteenua Companion actually exposes.
